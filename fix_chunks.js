...
const chunksDir = path.join(__dirname, '_next', 'static', 'chunks');
const files = fs.readdirSync(chunksDir).filter(f => f.endsWith('.js'));

// We will revert the generic "Shader" replacements that might have broken identifiers.
const reverts = [
  {
    from: /Avukat ERKAN UYSAL Hukuk Bürosu/g,
    to: "Shader Sweden AB"
  },
  {
    from: /Erkan Uysal/g,
  ...
  {
    from: /Avukat ERKAN UYSAL/g,
    to: "Shader"
  }
];

// After reverting, we can apply SAFE replacements (only when surrounded by quotes)
// We will look for "Shader" or 'Shader' or `Shader` and replace with Avukat ERKAN UYSAL
// But actually, to be completely safe and get the site working immediately, let's just revert.
// The index.html already has the main content.
...
for (const file of files) {
  ...
  // Now, safely replace the strings.
  // Next.js chunks often use double quotes for strings in JSON, or single quotes in JS.
  newContent = newContent.replace(/"Shader Development Studio"/g, '"Avukat ERKAN UYSAL"');
  newContent = newContent.replace(/"Shader"/g, '"Avukat ERKAN UYSAL"');
  newContent = newContent.replace(/'Shader'/g, "'Avukat ERKAN UYSAL'");
  newContent = newContent.replace(/"Shader Sweden AB"/g, '"Avukat ERKAN UYSAL Hukuk Bürosu"');
  newContent = newContent.replace(/"Shader Sweden"/g, '"Erkan Uysal"');

  if (content !== newContent) {
    fs.writeFileSync(filePath, newContent, 'utf8');
    console.log(`Reverted and safely updated ${file}`);
  }
}

// Let's also do the same for index.html just in case there are identifiers there.
const indexPath = path.join(__dirname, 'index.html');
let indexContent = fs.readFileSync(indexPath, 'utf8');
let newIndexContent = indexContent;

for (const {from, to} of reverts) {
  newIndexContent = newIndexContent.replace(from, to);
}

// Safely apply again for index.html
newIndexContent = newIndexContent.replace(/>Shader Development Studio</g, '>Avukat ERKAN UYSAL<');
newIndexContent = newIndexContent.replace(/"Shader Development Studio"/g, '"Avukat ERKAN UYSAL"');
newIndexContent = newIndexContent.replace(/"Shader"/g, '"Avukat ERKAN UYSAL"');
newIndexContent = newIndexContent.replace(/>Shader</g, '>Avukat ERKAN UYSAL<');
newIndexContent = newIndexContent.replace(/"Shader Sweden AB"/g, '"Avukat ERKAN UYSAL Hukuk Bürosu"');
newIndexContent = newIndexContent.replace(/"Shader Sweden"/g, '"Erkan Uysal"');

// Re-apply other safe text replacements that don't involve the word Shader directly
...
console.log("Reverted index.html identifiers and reapplied safe text.");
